    ...
    public class OrificeSetup
    {
        ...
        public static void ValidateLights(Orifice orifice)
        {
            ...
            if(orifice.MasterTransform != null)
            {
                ...
                orifice.LightPosition.range = GeneralSetup.GetOrificeRangeId(orifice.Channel, orifice.Type);
                orifice.LightPosition.color = GeneralSetup.GetOrificeColor(orifice.Channel, (int)orifice.Type);
                ...
                if(orifice.UseNormalLight)
                {
                    ...
                    orifice.LightNormal.range = GeneralSetup.GetNormRangeId(orifice.Channel);
                    orifice.LightNormal.color = GeneralSetup.GetOrificeColor(orifice.Channel, 2);
        ...
        public static void SetupOrificeLayers(Orifice orifice, AnimatorController animator, Transform avatarRoot)
        {
            ...
            Func<string, int, float, AnimationClip> PenAnim = (string name, int activeKeyIndex, float strength) =>
            {
                ...
                CurveConfig[] curves = new CurveConfig[orifice.OpeningShapekeys.Length + 1];
                curves[orifice.OpeningShapekeys.Length] = new CurveConfig("", orifice.Param_Depth, typeof(Animator), CustomCurve((1, activeDepth)));
                for(int i = 0; i < orifice.OpeningShapekeys.Length; i++)
                {
            ...
            AnimationClip[] noWidthClips = new AnimationClip[orifice.OpeningShapekeys.Length + 1];
            AnimationClip[] fullWidthClips = new AnimationClip[orifice.OpeningShapekeys.Length + 1];
            BlendTree[] depthTrees = new BlendTree[orifice.OpeningShapekeys.Length + 1];
            for(int i = 0; i < orifice.OpeningShapekeys.Length + 1; i++)
            {
